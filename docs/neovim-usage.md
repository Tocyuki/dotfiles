...
| `K` | Hover表示 | カーソル下のシンボルの説明をフロート表示 |
| `<leader>ca` | Code Action | 利用可能なコードアクションを表示 |
...
| キーバインド | 機能 | 説明 |
|-------------|------|------|
| `<leader>bd` | バッファを閉じる | 現在のバッファを削除 |
| `<leader>bo` / `<Leader>K` | 他のバッファを閉じる | 現在のバッファ以外を削除 |
| `<C-s>` / `<leader>w` | ファイル保存 | 現在のファイルを保存 |
...
**プラグイン**: [fyler.nvim](https://github.com/A7Lavinraj/fyler.nvim)
...
| `q` | ツリーを閉じる | Fylerウィンドウを閉じる |

Fylerはファイルシステムをバッファのように編集できるファイルマネージャーです。

### Snacks Picker（ファジーファインダー）

**プラグイン**: [snacks.nvim](https://github.com/folke/snacks.nvim)

#### ファイル・Git操作

| キーバインド | 機能 | 説明 |
|-------------|------|------|
| `<C-f>` | ファイル検索 | プロジェクト内ファイル検索 |
| `gf` | Gitファイル検索 | Gitで管理されているファイルを検索 |
| `gs` | Git Status | Git変更があるファイルを検索 |
| `gb` | Git Blame | カーソル行のblame情報を表示 |
| `go` | Git Open | 現在のファイルをブラウザで開く |
| `ga` | 文字列検索 | Ripgrepによる文字列検索 |
| `gl` | Lazygit | SnacksターミナルでLazygitを開く |
...
- **pyright** - Python
- **typescript-tools.nvim** - TypeScript/JavaScript
- **gopls** - Go
...
| `:AutosaveToggle` | 自動保存切り替え | 自動保存の有効/無効を切り替える |
| `:AutosaveEnable` | 自動保存有効化 | 自動保存を有効にする |
| `:AutosaveDisable` | 自動保存無効化 | 自動保存を無効にし、保留中の保存タイマーを停止する |
